...
import { Copy, TrendingUp, DollarSign, Ticket, ArrowUpRight, LogOut } from "lucide-react";
import { useState } from "react";
import { logoutAffiliate } from "@/app/actions/affiliate";
...
export default function DashboardClient({ affiliate }: { affiliate: any }) {
    ...
    const myLink = `${baseUrl}/raffle/${affiliate.raffle?.id}?ref=${affiliate.id}`;
            ...
            <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-3xl font-black tracking-tight uppercase">
                        Olá, <span className="text-primary">{affiliate.name.split(' ')[0]}</span>! 👋
                    </h1>
                ...
                <CardContent>
                    ...
                    <div className="mt-6 flex flex-wrap gap-4 text-sm font-medium border-t border-primary/10 pt-4">
                        <div className="bg-white/50 dark:bg-black/20 px-3 py-1.5 rounded-md border border-primary/20">
                            Sorteio Vinculado: <strong className="text-zinc-900 dark:text-zinc-100">{affiliate.raffle?.title}</strong>
                        </div>
                        <div className="bg-white/50 dark:bg-black/20 px-3 py-1.5 rounded-md border border-primary/20 text-success font-bold">
                            Sua Comissão: {Number(affiliate.commissionPercentage)}%
                        </div>
                    </div>
                </CardContent>
            </Card>
                ...
                <Card className="border-success/20 shadow-sm relative overflow-hidden">
                    ...
                    <CardContent>
                        <div className="text-4xl font-black text-success tracking-tight">
                            R$ {Number(affiliate.revenueGenerated).toFixed(2).replace('.', ',')}
                        </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-zinc-100 dark:border-zinc-800">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">Comprou 50 cotas ({affiliate.raffle?.title})</p>
                            </div>
                        ...
                        <div className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-zinc-100 dark:border-zinc-800">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">Comprou 100 cotas ({affiliate.raffle?.title})</p>
                            </div>
                            <div className="text-right">
                        ...
                        <div className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-zinc-100 dark:border-zinc-800">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">Comprou 10 cotas ({affiliate.raffle?.title})</p>
                            </div>
                            <div className="text-right">
