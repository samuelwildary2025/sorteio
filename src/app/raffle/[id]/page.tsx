...
import { ArrowLeft, Clock, Minus, Plus, ShoppingCart, ShieldCheck, Trophy } from "lucide-react";
...
export default function RaffleDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    // Find the prize using the ID from the URL
    const resolvedParams = use(params);
    const prizeIndex = parseInt(resolvedParams.id, 10);
    const prize = mockPrizes.find(p => p.id === prizeIndex);
            ...
            <div className="sticky top-16 z-40 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800">
                <div className="container mx-auto px-4 h-14 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                        <Clock className="h-3 w-3" /> Sorteio em: <span className="text-primary">2 dias</span>
                    </div>
                </div>
            </div>
