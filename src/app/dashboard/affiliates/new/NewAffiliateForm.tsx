...
export default function NewAffiliateForm({ prizes }: { prizes: PrizeOption[] }) {
    const router = useRouter();
    const [state, formAction] = useFormState(createAffiliate, initialState);
    const [cpf, setCpf] = useState("");

    const activePrizes = prizes.filter(p => p.status !== "draft");
                                ...
                                <div className="space-y-2">
                                    ...
                                    >
                                        ...
                                        {activePrizes.length === 0 && (
                                            <option value="" disabled>Nenhum sorteio cadastrado no banco</option>
                                        )}
